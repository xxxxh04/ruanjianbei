      ...
      <div id="selectmenu">
        ...
        <div id="selectdiffculty">
          <el-select v-model="selectDiffculty" placeholder="题目难度">
            <el-option v-for="item in diffcultyOptions" :key="item.value" :label="item.label" :value="item.value" />
          </el-select>
        </div>
        <div id="laberShow">
          <el-button id="laberShowbutton" type="primary" @click="showModal">标签</el-button>
          <el-dialog title="多选按钮弹框" v-model="dialogVisible" width="30%">
            <div id="selectLabel" class="select-label-container">
              <div class="label-group">
                ...
                <el-button v-for="laberOption in laberOptions1" :key="laberOption.value"
                  :type="isChecked(laberOption.value) ? 'success' : 'default'" @click="toggleColor(laberOption.value)">
                  {{ laberOption.label }}
                </el-button>
              </div>
              <div class="label-group">
                <span class="selectLabelSpan">算法</span>
                <el-button v-for="laberOption in laberOptions2" :key="laberOption.value"
                  :type="isChecked(laberOption.value) ? 'success' : 'default'" @click="toggleColor(laberOption.value)">
                  {{ laberOption.label }}
                </el-button>
              </div>
        ...
        <div id="searchInput">
          ...
          <div id="searchInputText">
            <el-input v-model="searchInput" placeholder="请输入搜索内容" @input="search" clearable />
          </div>
        </div>
      </div>
      <div id="showSelect">
        ...
        <el-button class="showSelectButton" v-for="checkedLaber in checkedLabers" :key="checkedLaber">
          {{ checkedLaber }}
        </el-button>
      </div>
    </div>
